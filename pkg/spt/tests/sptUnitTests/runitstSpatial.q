...
test.stDist <- function(){
  ...
  colnames(crd) <- c("long","lat")
  bbox <- cbind( apply(crd,2,min), apply(crd,2,max) )
  colnames(bbox) <- c("min","max")
  this <- new(testClassName, s.id=as.integer(1:n), coords=crd, bbox=bbox, proj4string = CRS(as.character(NA)) )
  
  checkEquals(sum(stDist(this,"euclidean")-stdisteu2d),0)
  checkTrue(sum(stDist(this,"earth")-stdistea2d)<.1)
  ...
  colnames(bbox) <- c("min","max")
  this <- new(testClassName, s.id=as.integer(1:n), coords=crd, bbox=bbox, proj4string = CRS(as.character(NA)) )
  checkEquals(sum(stDist(this,"euclidean")-stdisteu3d),0)
  ## should fail because the column names do not include lat and long
  checkException(stDist(this,"earth"))
}
