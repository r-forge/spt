...
test.SpatialTemporalDataFrame <- function(){
  testClassName <- "SpatialTemporalDataFrame"
  stdf <- data.frame( times=c("2008-09-29","2009-01-14","2012-12-12"), lat=c(31,33,33), long=c(-81,-80,-80), obs=c(50, 71, 10) )
  location.col <- 2:3
  time.col <- 1
  tmp <- SpatialTemporalDataFrame(stdf, location.col, time.col)  
...
test.subsetTemp <- function(){
  ...
  location.col <- 2:3
  time.col <- 1
  tmp <- SpatialTemporalDataFrame(stdf, location.col, time.col, tformat="%Y-%m-%d")    
...
test.subsetSpat <- function(){
  ...
  location.col <- 2:3
  time.col <- 1
  tmp.stdf <- SpatialTemporalDataFrame(stdf, location.col, time.col, tformat="%Y-%m-%d")
...
test.stDist <- function(){
  ...
  checkEquals(stDist(tmp,type="temporal",units="days"), stdistDays)
  checkEquals(stDist(tmp,"temporal","weeks"), stdistWeeks)
  
  load("testspatialpointsdataframe.Rdata")
  checkEquals(stDist(tmp,"spatial","euclidean"), stdisteu)
  checkEquals(stDist(tmp,"spatial","earth"), stdistea)
...
test.stJoin <- function(){
  ...
  location.col <- 3:2
  time.col <- 1
  s1 <- SpatialTemporalDataFrame(stdf, location.col, time.col)
  ...
  location.col <- 3:2
  time.col <- 1
  s2 <- SpatialTemporalDataFrame(stdf, location.col, time.col)
