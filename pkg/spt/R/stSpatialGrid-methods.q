...
## Constructor
stSpatialGrid <- function(grid, grid.index, coords, bbox=matrix(NA), proj4string = CRS(as.character(NA)), s.id=as.integer(1:nrow(coords)) ){
  spsp <- SpatialGrid(coords, proj4string, bbox)
  return(new("stSpatialGrid", s.id=s.id, grid=grid, grid.index=grid.index, coords=as.matrix(spsp@coords), bbox=spsp@bbox, proj4string = spsp@proj4string))
}
...
setReplaceMethod("setSpatialGrid", signature("stSpatialGrid"),
          function(object,value){
            validObject(value)
...
setMethod("stSubset", signature(x="stSpatialGrid", bounds="integer"),
          function(x,bounds){
...
## FIX TBD grid, grid.index need to be updated.
setMethod("stSubset", signature(x="stSpatialGrid", bounds="numeric"),
          function(x,bounds){
...
## FIX TBD grid, grid.index need to be updated.
setMethod("stSubset", signature(x="stSpatialGrid", bounds="matrix"),
          function(x, bounds){
            ...
            rownames(x@bbox) <- colnames( coordinates(x))
            if (length(new.members)==1) {
              return( stSpatialGrid(x@s.id[new.members], coords=t(as.matrix(coordinates(x)[new.members,])),
                                      bbox=x@bbox, proj4string=x@proj4string) )
            } else {
              return( stSpatialGrid(x@s.id[new.members], coords=coordinates(x)[new.members,],
                                      bbox=x@bbox, proj4string=x@proj4string))
            }
          }
          )
...
setMethod("stUpdate",c(st="stSpatialGrid", new.id="integer"),
          function(st, new.id){
...
setMethod("stDist", signature(sp="SpatialGrid", type="character"),
...
## FIX TBD grid, grid.index need to be updated.
setMethod("stJoin", signature(x="stSpatialGrid", y="stSpatialGrid"),
          function(x,y){
            ## Fix, TBD check bbox, proj4str and more??
            x.c <- coordinates(x)
            y.c <- coordinates(y)
            if (ncol(x.c) != ncol(y.c))
              stop("You can only join stSpatialGrid objects which both have the same number of coordinates")
            sp <- SpatialGrid(unique( rbind(x.c, y.c)))
            ...
            x@s.id <- new.sids
            x@bbox <- sp@bbox
            return(list(stSpatialGrid=x, x.id.map=x.id.map, y.id.map=y.id.map) )
          }
          )
